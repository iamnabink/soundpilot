// SoundPilot/Shortcuts/MenuBarPopupController.swift
import AppKit
import os

/// Toggles the SoundPilot menu-bar popup from outside the SwiftUI scene chain
...
final class MenuBarPopupController: MenuBarPopupControlling {
    private static let logger = Logger(
        subsystem: "com.whoamie.soundpilot",
        category: "MenuBarPopupController"
    )

    /// Accessibility title used to identify SoundPilot's status item among any
    ...
    /// argument we pass to `FluidMenuBarExtra(...)` in `SoundPilotApp`.
    private let accessibilityTitle: String

    init(accessibilityTitle: String = "SoundPilot") {
