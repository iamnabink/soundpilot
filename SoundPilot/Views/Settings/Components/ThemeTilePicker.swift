// SoundPilot/Views/Settings/Components/ThemeTilePicker.swift
import SwiftUI
            ...
            ThemePreviewMockup(preference: preference)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 6, style: .continuous)
                        .strokeBorder(
                            isSelected ? DesignTokens.Colors.selectionBorder : DesignTokens.Colors.separator,
...
/// Miniature macOS window drawn inside a theme tile.
///
/// The grays and traffic-light dots below are deliberately literal and
/// un-themed: the tile's job is to show what *macOS* looks like in Light vs
/// Dark so the user can pick an appearance. Tinting them with the SoundPilot
/// palette would make both tiles look the same and destroy the affordance.
    ...
    var body: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 5, style: .continuous)
                .fill(DesignTokens.Colors.contentBackground)
                .overlay {
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        .strokeBorder(DesignTokens.Colors.separator, lineWidth: 0.5)
                }
...
private struct MockDeviceRow: View {
    private static let element = DesignTokens.Colors.textTertiary
    private static let badge = DesignTokens.Colors.accentPrimary.opacity(0.6)
...
#Preview("Popup Size Tiles") {
    ...
    return VStack(spacing: 24) {
        VStack(alignment: .leading, spacing: 8) {
            Text("Dark").font(.caption).foregroundStyle(DesignTokens.Colors.textSecondary)
        ...
        VStack(alignment: .leading, spacing: 8) {
            Text("Light").font(.caption).foregroundStyle(DesignTokens.Colors.textSecondary)
