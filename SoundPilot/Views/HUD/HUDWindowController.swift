// SoundPilot/Views/HUD/HUDWindowController.swift
import AppKit
import SwiftUI
import os
...
final class HUDWindowController: MediaKeyHUDPresenting {
    ...
    private let logger = Logger(subsystem: "com.whoamie.soundpilot", category: "HUDWindowController")
    ...
    func showPerAppNotControlledHUD(displayName: String?, bundleID: String?, icon: NSImage?) {
        let title = displayName?.nilIfEmpty
            ?? bundleID?.nilIfEmpty
            ?? "SoundPilot isn't controlling this app yet"
        presentPerApp(
            icon: icon,
            title: title,
    ...
    private func postPerAppAccessibilityAnnouncement(panel: NSPanel, title: String, content: PerAppHUDContent) {
        ...
        case .notControlled:
            description = "\(title), not controlled by SoundPilot"
        }
        NSAccessibility.post(
            element: panel,
            notification: .announcementRequested,
...
private struct PerAppHUD: View {
    ...
    private var subtitleText: String? {
        if case .notControlled = content { return "Not controlled by SoundPilot" }
        return nil
    }
    ...
    private var accessibilityDescription: String {
        ...
        case .notControlled:
            return "\(title), not controlled by SoundPilot"
        }
    }
