// SoundPilot/Views/Components/DevicePicker.swift
import SwiftUI
...
struct DevicePicker: View {
        ...
        deviceIcon(firstDevice)
            .overlay(alignment: .bottomTrailing) {
                Text("\(count)")
                    ...
                    .foregroundStyle(DesignTokens.Colors.badgeSelectedForeground)
    ...
    private var iconOnlyBackgroundFill: Color {
        if isExpanded {
            return DesignTokens.Colors.controlFillHover
        } else if isButtonHovered {
            return DesignTokens.Colors.controlFill
        } else {
            return Color.clear
        }
    }
...
private struct DevicePickerRow: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .font(.system(size: 11))
            .foregroundStyle(isDisabled ? DesignTokens.Colors.disabledForeground : DesignTokens.Colors.textPrimary)
            .padding(.horizontal, 8)
            .frame(height: 26)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: 5)
                    .fill(isHovered && !isDisabled ? DesignTokens.Colors.selectionFill : Color.clear)
            )
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
    ...
    private var itemIcon: some View {
        ...
        case .systemAudio:
            Image(systemName: "globe")
                .font(.system(size: 13))
                .frame(width: 16)
                .foregroundStyle(isDisabled ? DesignTokens.Colors.disabledForeground : DesignTokens.Colors.textSecondary)
        case .device:
