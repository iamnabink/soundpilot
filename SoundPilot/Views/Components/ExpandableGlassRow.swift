// SoundPilot/Views/Components/ExpandableGlassRow.swift
import SwiftUI
...
struct ExpandableGlassRow<Header: View, ExpandedContent: View>: View {
    ...
    var body: some View {
        ...
        // On macOS 26 the lit state is a Liquid Glass pill, which is what makes
        // an active row read as a raised control rather than a tinted rectangle.
        // Pre-26 it stays the flat `hoverSurface` fill. Either way the fill only
        // exists while lit, so an idle row is still transparent glass.
        .background {
            if isHovered || isExpanded || isFocused {
                GlassPanel(
        ...
        // Keyboard focus adds a ring on top of the fill so it stays
        // distinguishable from a hovered or expanded row.
        .overlay {
            RoundedRectangle(cornerRadius: DesignTokens.Dimensions.rowRadius)
                .strokeBorder(isFocused ? DesignTokens.Colors.focusRing : Color.clear, lineWidth: 1)
                .padding(.vertical, 1)
                .allowsHitTesting(false)
        }
        .onHover { hovering in
