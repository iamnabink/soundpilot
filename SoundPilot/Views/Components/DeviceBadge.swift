// SoundPilot/Views/Components/DeviceBadge.swift
import SwiftUI
import AppKit

/// Circular tinted badge that replaces the leading radio button on a device row.
/// Selected state uses the brand gradient (`badgeGradientStart` → `badgeGradientEnd`,
/// blue into violet, echoing the app icon). Unselected state uses a monochrome fill
/// from `DesignTokens.Colors.deviceBadgeMonoFill`.
///
...
struct DeviceBadge: View {
            ...
            if isSelected {
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                DesignTokens.Colors.badgeGradientStart,
                                DesignTokens.Colors.badgeGradientEnd
                            ],
    ...
    private var glyphForeground: Color {
        isSelected
            ? DesignTokens.Colors.badgeSelectedForeground
