// SoundPilot/Views/DesignSystem/VisualEffectBackground.swift
import SwiftUI
import AppKit
...
extension View {
    ...
    /// The menu bar popup's glass surface.
    ///
    /// Differs from `darkGlassBackground()` in one decisive way: it does **not**
    /// add an NSVisualEffectView. FluidMenuBarExtra already installs one as the
    /// window's content view, so the old modifier was stacking a second
    /// `.behindWindow` material inside the first and then painting a half-opaque
    /// tint on top of both. Three layers of frosting is why the popup read as
    /// flat milk instead of glass.
    ///
    /// What is left is one material (the window's), a light tint, and a
    /// specular highlight along the top edge. The highlight is a soft gradient
    /// rather than a stroked border on purpose: the system rounds the panel's
    /// corners for us at a radius we do not control, and a hard rim drawn at
    /// the wrong radius would clip visibly at the corners.
    ///
    /// Previews keep using `darkGlassBackground()`, which still supplies its own
    /// material, because a preview has no menu bar window behind it.
    func popupGlassBackground() -> some View {
        background {
            ZStack(alignment: .top) {
                DesignTokens.Colors.popupGlassTint

                LinearGradient(
                    colors: [
                        DesignTokens.Colors.popupSpecular,
                        DesignTokens.Colors.popupSpecular.opacity(0)
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )
