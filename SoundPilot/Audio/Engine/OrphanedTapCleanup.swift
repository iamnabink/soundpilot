// SoundPilot/Audio/Engine/OrphanedTapCleanup.swift
import AudioToolbox
import os

private let logger = Logger(subsystem: "com.whoamie.soundpilot", category: "OrphanedTapCleanup")

/// Scans CoreAudio for orphaned SoundPilot aggregate devices and destroys them.
/// Orphans occur when SoundPilot crashes or is force-killed (`kill -9`), leaving
/// aggregate devices with `.mutedWhenTapped` process taps that silently mute apps.
enum OrphanedTapCleanup {
    /// Destroys any aggregate devices named "SoundPilot-*" left over from a previous session.
    ...
    static func destroyOrphanedDevices() {
        ...
        for device in devices {
            ...
            guard let name = try? device.readDeviceName(),
                  name.hasPrefix("SoundPilot-") else { continue }
        ...
        if destroyedCount == 0 {
            logger.info("[CLEANUP] No orphaned SoundPilot devices found")
        } else {
