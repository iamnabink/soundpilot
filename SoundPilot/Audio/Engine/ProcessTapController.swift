// SoundPilot/Audio/Engine/ProcessTapController.swift
import AudioToolbox
import Foundation
import os
...
final class ProcessTapController: ProcessTapControlling {
    ...
    init(
        ...
        self.logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.whoamie.soundpilot", category: "ProcessTapController(\(app.name))")
    }
    ...
    /// Resolved plan for SoundPilot's private wrapping aggregate: which hardware sub-devices
    ...
    private func buildAggregateDescription(outputUIDs: [String], tapUUID: UUID, name: String) -> [String: Any] {
        ...
        let plan = Self.planAggregate(
            ...
            expand: { uid in
                ...
                // be flattened into SoundPilot's aggregate (nested aggregates aren't supported).
    ...
    private func createSecondaryTap(for outputUIDs: [String]) throws {
        ...
        let description = buildAggregateDescription(
            outputUIDs: outputUIDs,
            tapUUID: tapDesc.uuid,
            name: "SoundPilot-\(app.id)-secondary"
        )
