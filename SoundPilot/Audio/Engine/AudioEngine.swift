// SoundPilot/Audio/Engine/AudioEngine.swift
import AudioToolbox
import Foundation
import os
...
final class AudioEngine {
    ...
    /// The last output default confirmed by SoundPilot (user change or programmatic switch).
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.whoamie.soundpilot", category: "AudioEngine")
    ...
    /// Hide an active app so SoundPilot ignores it entirely. Persists the ignore,
    ...
    /// Restores the default to `lastConfirmedDefaultUID` (what the user/SoundPilot intended).
    ...
    private func handleDefaultDeviceChanged(_ newDefaultUID: String) {
        ...
        if case .pendingAutoSwitch(let pendingUID, let timeoutTask) = outputPriorityState {
            // Check echoes FIRST — SoundPilot's own changes (UI, restoreConfirmedDefault)
            // create echoes. Consuming before Case 1 ensures SoundPilot UI changes aren't
    ...
    private func handleDefaultInputDeviceChanged(_ newDefaultInputUID: String) {
        ...
        // Restore the locked device — any change outside SoundPilot's UI is either
        // macOS auto-switch or System Settings, and the lock should hold either way.
        // Users change the lock via SoundPilot's UI (setLockedInputDevice).
    ...
    /// Called when user explicitly selects an input device (via SoundPilot UI).
