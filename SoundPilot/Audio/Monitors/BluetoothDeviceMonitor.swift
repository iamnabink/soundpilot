// SoundPilot/Audio/Monitors/BluetoothDeviceMonitor.swift
import AppKit
import IOBluetooth
import os
...
final class BluetoothDeviceMonitor {
    ...
    private nonisolated static let btQueue = DispatchQueue(label: "com.whoamie.soundpilot.bluetooth")
    ...
    /// via SoundPilot) are removed. If a SoundPilot-initiated connection is in flight,
    ...
    func notifyDeviceAppearedInCoreAudio() {
        if !connectingIDs.isEmpty {
            ...
            // Auto-connected device (not via SoundPilot) — still need to refresh
