// SoundPilot/Settings/SettingsManager.swift
import Foundation
import os
...
final class SettingsManager {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.whoamie.soundpilot", category: "SettingsManager")
    ...
    init(directory: URL? = nil) {
        let baseDir = directory ?? FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("SoundPilot")
    ...
    func ignoreApp(_ identifier: String, info: IgnoredAppInfo) {
        ...
        settings.pinnedApps.remove(identifier)
        settings.pinnedAppInfo.removeValue(forKey: identifier)
        // Clear per-app settings — SoundPilot won't interact with this app
