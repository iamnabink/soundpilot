// SoundPilotTests/TargetAppResolverTests.swift
import Testing
import Foundation
import AppKit
...
struct TargetAppResolverTests {
    private static let ownBundleID = "com.whoamie.soundpilot"

    // MARK: - No-audible fallback (Phase 1 behavior preserved)

    @Test("resolveTargetBundleID returns nil when nothing has activated and SoundPilot is frontmost")
    ...
    @Test("resolveTargetBundleID returns frontmost when not SoundPilot and no candidates")
    func returnsFrontmostWhenNotSoundPilot() {
        let resolver = TargetAppResolver(
            ownBundleID: Self.ownBundleID,
    ...
    @Test("resolveTargetBundleID falls back to last cached non-SoundPilot ID when SoundPilot is frontmost")
    func soundPilotFrontmostUsesCachedFallback() {
        let resolver = TargetAppResolver(
            ownBundleID: Self.ownBundleID,
            frontmostBundleIDProvider: { Self.ownBundleID }
        )
    ...
    @Test("activation by SoundPilot does not overwrite the cached fallback")
    func soundPilotActivationDoesNotPoisonCache() {
        let resolver = TargetAppResolver(
            ownBundleID: Self.ownBundleID,
            frontmostBundleIDProvider: { Self.ownBundleID }
        )
    ...
    @Test("filters SoundPilot's own bundle ID out of candidates")
