...
SoundPilot can apply headphone-specific frequency response corrections using profiles from the [AutoEQ](https://github.com/jaakkopasanen/AutoEq) project. This compensates for your headphones' natural frequency curve, giving you a flatter, more accurate sound.

## How It Works

Every pair of headphones colors the sound differently. Some boost bass, others are harsh in the treble. AutoEQ measures these deviations and generates corrective EQ filters. SoundPilot applies these filters per-device, so each pair of headphones gets its own correction profile.

Corrections are applied on top of SoundPilot's 10-band EQ, so you can still tweak the sound to your taste after applying a profile.

## Browse Built-in Profiles

1. Click the **wand icon** next to any headphone device in SoundPilot
...
SoundPilot accepts [EqualizerAPO](https://sourceforge.net/projects/equalizerapo/) ParametricEQ.txt files:
...
- **Built-in search** — The easiest way. Thousands of headphones are available directly in SoundPilot.
- **[autoeq.app](https://www.autoeq.app/)** — Web-based tool with more options. Select **EqualizerAPO ParametricEq** as the equalizer app, download the file, and import it into SoundPilot.
